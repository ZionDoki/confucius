...
的几篇」。Agent 执行 OpenAlex 检索后，结果在对话中的文献卡片里出现；空白任务没有常驻
文献入口。卡片显示查询、实际结果池和候选预览，点击「查看文献」展开候选与结果池。

当这张卡片完全滚出对话可视区域，输入框上方才出现「文献」胶囊。点击胶囊在输入框上方
展开浮层，不跳动对话位置；候选选择、筛选、摘要展开和列表滚动位置保留。点击「收起」、
外部或按 Escape 关闭浮层，后台工作继续；「回到对话位置」定位原卡片。候选与结果池
标签支持左右方向键切换。切换任务时关闭当前浮层，显示所选任务自己的文献状态。

卡片内的「调整检索条件 · 联网」支持关键词、年份、开放获取及排序，另可继续加载已有
...
只收集文献不会自动开始研读。仅当此前研究明确等待这些全文时，补齐会唤醒原有等待。
关闭工作区不会停止下载；停止下载后可以重试。重启会保留结果、候选和获取记录，中断
的下载显示为可重试，不自动重新执行研究。

## 研究子 Agent

主 Agent 可以按需委派检索、逐篇阅读或方法比较。最多同时执行两个，其余排队。
...
时间线中的简短入口可打开只读气泡，查看公开进度、工具活动、结果或错误。气泡提供停止
和重试，没有独立聊天框；点击外部或 Escape 关闭，后台工作继续。子 Agent 不出现在
