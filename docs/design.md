...
- 文献从对话发起：Agent 检索后，卡片出现在对应的时间线位置；空任务不显示文献入口。
  卡片显示查询、OpenAlex／年份／任务池数量、两条简短预览和候选状态。细节默认收起，
  查询条件与本地筛选作为卡片内的次级入口，不挤占输入区。
- 卡片采用 `paper` 底色、14px 圆角和一条 `line` 结构边界；标题 14px、内容 13px、
  元信息 12px。常规内边距 16px／20px，紧凑内边距 12px；按钮复用公共样式。
  列表用留白组织，论文行不另加装饰边框；复选框对齐标题第一行。
- 当前文献卡片完全离开对话可视区域后，输入框正上方居中出现胶囊，显示结果池、候选及
  已确认文献的待补全文数。卡片仍有可见部分时不显示胶囊，不与「回到最新」重叠。
  胶囊高至少 34px、12px 文字、圆角 24px，使用公共浮层底色与轻阴影。
- 点击胶囊，在其上方展开最大宽度 680px 的不透明浮层；外层宽度不超过输入卡片的
  880px，窄窗口两侧至少留 8px，高度由时间线可用空间决定。浮层采用 `elevated` 底色、
  14px 圆角与公共阴影，头尾固定、列表内部滚动，输入框始终可用。
- 卡片和浮层共用同一候选编辑器。展开／关闭不改变对话滚动位置；切换显示位置保留
  勾选、筛选、摘要展开和列表滚动位置；浮层打开期间暂停主时间线自动跟随新消息。
  只有「回到对话位置」主动定位卡片，卡片回到
  可视区域即收起胶囊。点击外部和 Escape 关闭，不停止后台工作；关闭时不抢占外部焦点。
- 候选与结果池标签支持方向键，焦点清晰。确认范围原位显示新增、移除与获取范围，
  「确认并获取全文」授权整个批次；版本变化后必须重新核对。全文状态独立于候选勾选。
  PDF 拖入区使用结构性虚线边界，拖入时突出对应目标。
- 子 Agent 在委派发生的位置显示图标、名称和状态的简短透明入口，更新不移动位置；
  不增加侧栏任务或聊天输入。点击打开最大宽度 420px 的只读气泡，沿用文献浮层配色、
  圆角与公共按钮，四周至少留 8px。关闭不取消任务；Escape 与外部点击均可关闭。
