...
try {
  ...
  await evaluate(`qa.run=host.subagents().run(qa.children[0].id);qa.serial=0;qa.emit=(type,payload)=>qa.run.event({id:'fixture-'+(++qa.serial),sessionId:qa.run.task.id,turnId:qa.run.task.run.id,ts:Date.now(),type,payload});
    for(let i=0;i<40;i++){qa.emit('tool_requested',{callId:'pages'+i,toolName:'get_pages',args:{page:i+1,libraryID:1,key:'SYNTHETIC'}});qa.emit('tool_progress',{callId:'pages'+i,message:'读取第 '+(i+1)+' 页'});qa.emit('tool_result',{callId:'pages'+i,result:{ok:true,toolName:'get_pages',data:{page:i+1,text:'Evidence '+i+'：本条仅为隔离测试材料，用于核对完整输入与工具结果。'}}});}
  ...
  await wait(
    `return d.querySelectorAll('.confucius-subagent-trace-item[data-kind=tool]').length===40;`,
  );
  assert.equal(
    await evaluate(
  ...
  await evaluate(
    `const row=d.querySelector('.confucius-subagent-trace-item[data-kind=tool]');row.open=true;row.querySelector('.confucius-subagent-raw').open=true;const scroll=d.querySelector('.confucius-subagent-scroll');scroll.scrollTop=0;return true;`,
  );
  await wait(`return !!d.querySelector('.confucius-subagent-raw[open] pre');`);
  const trace = await evaluate(
    `const row=d.querySelector('.confucius-subagent-trace-item[data-kind=tool]');return {text:row.textContent,records:d.querySelector('.confucius-subagent-filter span').textContent,archives:d.querySelectorAll('.confucius-subagent-details > div > details').length};`,
  );
  assert.match(trace.text, /Evidence 0/);
  assert.match(trace.text, /"page": 1/);
  assert.match(trace.text, /tool_requested/);
  ...
  await wait(
    `return [...d.querySelectorAll('.confucius-subagent-trace-item > summary')].some(s=>s.textContent.includes('get_outline'));`,
  );
  const continuity = await evaluate(
    `return {scroll:d.querySelector('.confucius-subagent-scroll').scrollTop,before:qa.scroll,main:d.querySelector('.confucius-timeline-pane').scrollTop,mainBefore:qa.mainScroll,open:!!d.querySelector('.confucius-subagent-trace-item[open] .confucius-subagent-raw[open]')};`,
  );
  assert.ok(
  ...
  const restored = await evaluate(
    `return {shell:d.getElementById('confucius-subagent-popup')===qa.popup,pane:d.querySelector('.confucius-subagent-pane')===qa.firstPane,query:d.querySelector('.confucius-subagent-filter input').value,scroll:d.querySelector('.confucius-subagent-scroll').scrollTop,before:qa.switchScroll,open:!!d.querySelector('.confucius-subagent-trace-item[open] .confucius-subagent-raw[open]'),receipt:d.querySelector('.confucius-subagent-details > div > details pre').textContent.endsWith('END-OF-RECEIPT'),previousDisabled:d.querySelector('.confucius-subagent-navigation [data-direction=previous]').disabled};`,
  );
  assert.equal(
    restored.shell &&
      restored.pane &&
  ...
  await evaluate(
    `qa.pending.splice(0,1)[0].resolve({end:'stop',text:'方法比较已完成。此测试未访问个人文库或在线模型。'});return true;`,
  );
  await wait(
  ...
  for (const [width, theme, height] of [
    ...
    await evaluate(
      `const s=d.querySelector('.confucius-subagent-scroll');s.scrollTop=0;d.querySelector('.confucius-subagent-trace-item[data-kind=tool]').open=true;return true;`,
    );
    await capture(
