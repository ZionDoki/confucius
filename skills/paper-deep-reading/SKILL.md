...
Unless the user requests a different format or length, write a focused report of about 1,500-2,500 Chinese characters (or 600-1,000 English words): a short answer to the research question; the central method choices; one evidence table with the 4-6 decisive findings; and 3-5 material assumptions or limitations. Each evidence row must give the result with its denominator and experiment population, a source-page link, and what it supports / leaves untested. Put each number in that table only; do not repeat the full results in prose and again in a conclusion. Detail is useful when it changes the reader's interpretation. Do not copy tool identifiers, implementation metadata or the full annotation inventory into the reading report; end with a short annotation outcome summary.
...
Give each substantive result and limitation a physical-page citation in the report body; annotation IDs alone are not source citations. For deductions or absent measurements, state explicitly that they are your assessment or were not reported. Remove claims you cannot substantiate. Save the corrected report under the same artifact ID with status ready, and make the annotation artifact agree with actual saved comments and receipts.
